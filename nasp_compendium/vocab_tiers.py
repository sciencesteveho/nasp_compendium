...
Classifies every node/term used in a draft into one of three tiers
(canonical, proposed, drift) and produces blocking errors and non-blocking
warnings. Intended to be called from `compendium validate`. The functions here
are pure over plain dicts and sets so they can be unit-tested without the
compendium internals; loaders read the human-owned `vocabulary.yaml` and the
draft's own inline `proposed_terms` block.
"""
...
def classify_term(
  ...
  return TermVerdict(
      ...
      blocking=True,
      message=(
          f"'{term}' is not canonical and has no proposal; either map it to a "
          "canonical node or add it to the draft's proposed_terms block."
      ),
  )
...
def validate_draft_terms(
  ...
  for term in sorted(set(draft_terms)):
    verdict = classify_term(term, vocabulary, proposed_terms, compendium_nodes)
    if verdict.blocking:
...
if __name__ == "__main__":
  ...
  canonical_verdict = classify_term("CGAS", vocab, draft_proposals, active_nodes)
