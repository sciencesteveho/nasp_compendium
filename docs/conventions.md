...
A single ruleset for curating per-paper `.md` files. Apply these to every paper.
If a paper makes you want to break a rule, raise it as a convention question
first; do not fork the vocabulary.
...
Tissue-agnostic — see anti-pattern below.
  ...
  - `cytoplasmic_chromatin_fragments` (CCF)
