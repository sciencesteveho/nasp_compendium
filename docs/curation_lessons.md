...
not the rules file; `conventions.md` remains the source of curation rules, and
`vocabulary.yaml` is the machine-readable controlled vocabulary. Read this file
before drafting edges to see what this workflow has historically gotten wrong or
needed human judgment to resolve.
...
intermediate the paper actually supports. See `conventions.md`, "No shortcut
edges across supported intermediates."
...
The controlled vocabulary now uses a tiered gate (`vocabulary.yaml`, with
`canonical` and `drift` sections; see `specs/tiered_vocab_spec.md`). Most of the
clarifications below are encoded there as canonical terms or drift replacements;
they are kept here for the reasoning.
...
context. Now fully captured as a rule in `conventions.md`, "What is NOT a node."

**Vocabulary additions go through the tier gate.** `vocabulary.yaml` is the
machine-readable source of truth and is human-owned. The agent may use a new
...
promotes, merges, or rejects it. The agent never writes to `vocabulary.yaml`.
...
- `vocabulary.yaml` still lists four drift terms (`CCF_formation`,
  `chromatin_accessibility`, `cytosolic_L1_cDNA_accumulation`, `L1_de-repression`)
  under `canonical.mechanisms` as well as in the `drift` table. Remove them from
  `canonical.mechanisms` so each term resolves to a single tier.
...
| Vocabulary infrastructure | 2026-05-27 | Vocabulary drift should be machine-detected and reviewed before terms enter `vocabulary.yaml`. |
